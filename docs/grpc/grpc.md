...
gRPC의 장점중 하나로 proto 파일을 활용한 타입 체크가 존재합니다.

**gRPC의 장점:**
- Protobuf 스키마를 통한 **컴파일 타임 타입 체크**
- 명확한 API 계약 정의
- 런타임 에러 감소 및 코드 안정성 향상

하지만 단점으로는 각 서비스가 **동일한 proto 파일을 중복 관리**해야 하며, 이로 인해 API 변경 시 **모든 서비스에서 proto 파일을 수동으로 동기화**해야합니다.
...
message PaymentRequest {
  int64 user_id = 1;
  int64 amount = 2;
  string payment_method = 3;  // ← 새 필드 추가
}

→ Concert 서비스, API Gateway 등 
  모든 클라이언트에서 proto 파일 수동 업데이트 필요
```

이를 해결하기 위해서 proto 파일을 별도의 저장소로 분리하여 관리하는 방법을 채택했습니다.

#### 저장소 분리

Proto 파일을 **독립된 GitHub 저장소**로 분리하고, **Maven 패키지로 배포**하여 각 서비스에서 의존성으로 사용하도록 구조를 개선했습니다.

![패키지화 사진](https://velog.velcdn.com/images/asdcz11/post/214d9eef-fc93-469f-ba92-ec687f636b0b/image.png)

```kotlin
// concert-service/build.gradle.kts
implementation("com.concert:concert-proto:1.0.3")
```

#### 적용 후 변경점

- proto 파일을 한 곳에서 관리 가능해졌습니다
- 버저닝을 통해 어떤점이 변경되었는지 확인 가능합니다
- GitHub ACTION을 통해서 패키지 배포 자동화가 가능합니다
