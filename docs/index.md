...
지금까지는 단일 인덱스를 통해서 쿼리 성능을 올렸지만, 이렇게 where 절에 2개 이상의 조건이 추가되면 인덱스를 여러개를 거는 복합키를 사용해서 쿼리 성능을 올릴 수 있습니다.

그러면 복합키를 어떻게 걸어야 성능적 이점을 얻을 수 있을지 알아보겠습니다.

복합키의 경우 인덱스의 순서를 어떻게 지정하는지에 따라서 쿼리 성능이 달라질 수 있습니다.
