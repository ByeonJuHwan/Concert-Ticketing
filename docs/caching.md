...
**그렇다면 `Request` 요청이 1번 2번 요청되는 상황은 캐시가 빠르다는건 알겠는데, 실제 몇천개의 요청이 왔을때 차이가 그렇게 심할까? 라는 의문이 들 수 있습니다.
...
부하테스트 도구는 `JMeter`를 사용하였고, 테스트 환경은 다음과 같습니다:
...
위 설정으로 5000명의 사용자가 10초동안 점진적으로 시스템에 접근하여 콘서트 예약가능한 날짜 조회 API 를 1번씩 요청하는 상황으로 부하테스트를 진행하였습니다.
(1만명의 테스트 시에는 커넥션풀 에러가 발생해서 절반으로 줄였습니다)
...
| 최대 응답 시간 | 148 ms |
| 처리량 | 500.1/sec |
| 수신 데이터량 | 7559.35 KB/sec |

## 결과 비교 및 분석

캐싱을 사용할 때와 사용하지 않을 때의 성능 차이는 다음과 같습니다:

- **평균 응답 시간**: 캐싱 사용 시 평균 응답 시간이 미사용 시에 비해 266배 빠릅니다.
- **최대 응답 시간**: 캐싱 사용 시 최대 응답 시간이 미사용 시에 비해 20.9배 빠릅니다.
- **처리량**: 캐싱 사용 시 처리량이 미사용 시에 비해 약 25% 더 많습니다.
- **수신 데이터량**: 캐싱 사용 시 수신 데이터량이 미사용 시에 비해 약 25% 더 많습니다.
...
따라서 이러한 토큰 관리 작업을 메모리 기반인 `Redis` 로 처리하면 빠른 속도로 대기열을 처리할 수 있고,
비즈니스 로직 외의 토큰 관리를 `Redis` 에서 처리함으로써 DB는 비즈니스 로직과 관련된 데이터만 처리하도록 할 수 있습니다.
...
JMeter를 사용한 부하 테스트 결과, DB 접근 로직에서 평균 TPS가 399.0/sec로 나타났습니다. 이는 1분 동안 약 24,000개의 요청을 처리할 수 있다는 것을 의미합니다.

각 유저가 호출할 것으로 예상되는 API 수는 다음과 같습니다:
- 콘서트 정보 조회 API
- 콘서트 좌석 조회 API
- 콘서트 예약 API

이로 인해 한 유저당 API 호출이 3회 발생한다고 가정할 경우, 실제로 1분에 처리할 수 있는 유저 수는 약 8,000명입니다. 따라서 10초당 처리 가능한 유저 수는 약 1,500명입니다.

이 분석을 토대로 10초에 1,500개의 `WaitingToken`을 `ActiveToken`으로 전환하는 것이 테스트 결과상 으로는 적절하지만, 실제 운영에서는 예상치 못한 변수를 고려하여 이보다 낮은 1,000개의 토큰 전환으로 시작하려고 합니다.
이는 초기 설정치로, 서비스 운영을 통해 수집되는 실제 데이터를 바탕으로 지속적인 튜닝을 하는것이 좋을 것 같습니다.

이렇게 접근하는 이유는 실제 트래픽 상황에서 발생할 수 있는 여러 변수들(네트워크 지연, 서버 자원의 변동 등)을 고려하여 실제 테스트 해본 수치보다 안정적으로 낮게 설정했습니다.
