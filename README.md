...
대용량 트래픽 대응을 위한 MSA 전환 과정과 발생한 기술적 문제 및 해결 방안을 정리했습니다.

**[MSA 전환 문서 보기](docs/msa/msa-convert.md)**
...
Kafka 기반 이벤트 발행으로 사용자 데이터를 각 서비스에 복제하여 서비스 간 결합도를 낮췄습니다.

**[최종적 일관성 구현](docs/msa/eventual-consistency.md)**

---

## Interceptor에서 API Gateway로 전환

MSA 환경에서 각 서비스에 중복되던 토큰 검증 로직을 API Gateway로 중앙화하여 유지보수성을 높였습니다.

**[API Gateway 토큰 검증](docs/msa/gateway.md)**
